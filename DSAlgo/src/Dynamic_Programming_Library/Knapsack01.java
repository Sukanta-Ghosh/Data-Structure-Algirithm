...
public class Knapsack01 {
    ...
    public int iterative(int[] A, int[] B, int C) {
         ...
         * you can choose from array of first i tiems.
         */
        int[][] dpTable = new int[n + 1][cap + 1];

        /*
         * i represents values and weights arr index
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= cap; j++) {
                 ...
                 * 
                 * Here weights[i - 1] and values[i - 1] taken as i - 1
                 * cuz, 0 element and 0 weights was considerd as dp[0][0],
                 * so i - 1 points to i th element in respective arrays
                 */
                int selection = 0;
