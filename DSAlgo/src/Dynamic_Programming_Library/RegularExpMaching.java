...
/* https://www.scaler.com/academy/mentee-dashboard/class/70873/assignment/problems/16/?navref=cl_pb_nv_tb */
public class RegularExpMaching {
    ...
    public int isMatchStr(String s, String p, int ns, int np) {
        ...
        /* If string empty, ten we will check in pattern * present or not */
        if (ns == 0) {
            for (int i = 0; i < np; i++) {
                if (p.charAt(i) != '*') {
                    return 0;
         ...
         * rec(ns, np - 1) => When from string nothing is included for pattern *
         * rec(ns - 1, np) => When from string last char is included for pattern *
         */
